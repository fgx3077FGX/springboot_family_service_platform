...
public class EstateController {
     ...
     * @param
     * @return
     * 此处应该完成的是楼宇的查询功能，但是现在数据表中没有任何楼宇的数据，因此在辨析的时候需要进行插入且返回插入的数据
     *
     * */
    ...
    public String updateBuilding(FcBuilding fcBuilding){
        ...
        if(result==0){
            ...
            return JSONObject.toJSONString(new ReturnObject("1","更新楼宇成功"));

        }else{
            System.out.println("result: "+result);
            return JSONObject.toJSONString(new ReturnObject("0","更新楼宇失败,请重新输入正确的楼宇信息"));
        }
